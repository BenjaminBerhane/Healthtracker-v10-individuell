...
const MealListItem = ({ meal, onEdit = () => {}, editButton = null }) => {
  ...
  return (
    <details className="bg-white shadow-md rounded p-4 border-1 border-accent/30" onClick={toggleDetails}>
      ...
      {showDetails && (
        <div className="flex flex-col items-start text-left">
          <p className={showDetails ? 'bg-secondary w-full p-2' : ''}>Datum: <span className="text-accent font-light">{meal.date}</span></p>
          <p className="text-gray-600 p-2">Protein: <span className="text-primary font-bold">{meal.protein}</span> g</p>
          <p className={showDetails ? 'bg-secondary w-full p-2' : ''}>Kolhydrat: <span className="text-primary font-bold">{meal.carbohydrate}</span> g</p>
          <p className="text-gray-600 p-2">Fett: <span className="text-primary font-bold">{meal.fat}</span> g</p>
          <p className={showDetails ? 'bg-secondary w-full p-2' : ''}>Kategori: <span className="text-accent text-light">{meal.category}</span></p>
        </div>
      )}
      { editButton &&
      <Button
